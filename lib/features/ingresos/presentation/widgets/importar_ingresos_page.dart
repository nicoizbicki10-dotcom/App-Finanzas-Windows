...
import '../../data/importar_ingresos_service.dart';
import '../../domain/ingreso.dart';
import '../providers/ingresos_provider.dart';

enum _ImportStep { input, analizando, confirmacion }

class ImportarIngresosPage extends ConsumerStatefulWidget {
  const ImportarIngresosPage({super.key});

  @override
  ConsumerState<ImportarIngresosPage> createState() =>
      _ImportarIngresosPageState();
}

class _ImportarIngresosPageState extends ConsumerState<ImportarIngresosPage> {
  final _service = ImportarIngresosService();
  ...
  List<IngresoParseado> _items = [];
  int _omitidos = 0;
  int _actualizados = 0;
  List<Ingreso> _actualizaciones = [];
  ...
  Future<void> _analizar() async {
    ...
    if (_tabIndex == 0 && _textCtrl.text.trim().isEmpty) {
      setState(() => _error = 'Pegá el texto antes de analizar.');
      return;
    }
    ...
    setState(() {
      _step = _ImportStep.analizando;
      _error = null;
    });

    try {
      List<IngresoParseado> parsed;
      if (_tabIndex == 0) {
        parsed = await _service.analizarTexto(
            apiKey: _apiKey!, texto: _textCtrl.text.trim());
      } else if (_pdfPath != null) {
        parsed =
            await _service.analizarPdf(apiKey: _apiKey!, pdfPath: _pdfPath!);
      } else {
        parsed =
      ...
      if (parsed.isEmpty) {
        setState(() {
          ...
          _error = 'No se encontraron ingresos en el contenido.';
        });
        return;
      }

      final (filtrados, omitidos, actualizaciones) =
          _filtrarDuplicadosFijos(parsed);

      if (filtrados.isEmpty && actualizaciones.isEmpty) {
        setState(() {
          _step = _ImportStep.input;
          _error = omitidos > 0
              ? 'Todos los ingresos fijos ya están registrados.'
              : 'No se encontraron ingresos en el contenido.';
        });
        return;
      }

      if (filtrados.isEmpty && actualizaciones.isNotEmpty) {
        final notifier = ref.read(ingresosNotifierProvider.notifier);
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              '${actualizaciones.length} ingreso${actualizaciones.length == 1 ? ' fijo actualizado' : 's fijos actualizados'} con nuevo monto.'),
  ...
  Future<void> _guardarTodo() async {
    ...
    for (final item in _items) {
      await notifier.agregar(item.toIngreso());
      if (item.medioPagoId != null) {
        await invNotifier.ajustarMontoLiquidez(item.medioPagoId!, item.monto);
      }
    }
    for (final ing in _actualizaciones) {
      await notifier.actualizar(ing);
    }
  ...
  (List<IngresoParseado>, int, List<Ingreso>) _filtrarDuplicadosFijos(
      List<IngresoParseado> items) {
    final repo = ref.read(ingresosRepositoryProvider);
    final existentesFijos =
        repo.getAll().where((i) => i.tipo == TipoIngreso.fijo).toList();
    final ahora = DateTime.now();
    final mesActual = DateTime(ahora.year, ahora.month);

    final resultado = <IngresoParseado>[];
    final actualizaciones = <Ingreso>[];
    int omitidos = 0;

    for (final item in items) {
      if (item.tipo != TipoIngreso.fijo) {
        resultado.add(item);
        continue;
      }
      ...
      final vencimiento = DateTime(
          existente.fecha.year,
          existente.fecha.month + existente.duracionMeses!);
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                  hintText: 'gsk_...', labelText: 'API Key'),
            ),
          ],
        ),
        actions: [
  ...
  String _parsearError(Object e) {
    if (e is DioException) {
      ...
      if (data is Map) {
        ...
        if (errorObj is Map) apiMsg = errorObj['message'] as String?;
      ...
      if (status == 400) {
        ...
        return 'Archivo no válido. Usá una imagen JPG, PNG o PDF.';
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Importar Ingresos con IA'),
        actions: [
          IconButton(
  ...
  Widget _buildInput() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'Pegá el texto de un recibo, extracto o adjuntá el archivo.',
            style:
          ...
          if (_tabIndex == 0) _buildTextoTab() else _buildArchivoTab(),
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText:
              'Pegá aquí el texto del recibo de sueldo, extracto bancario...',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          const Text('Seleccioná el comprobante o recibo',
              style: TextStyle(
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text('Identificando y clasificando ingresos',
              style: TextStyle(
  ...
  Widget _buildConfirmacion() {
    ...
    final timerColor =
        _segundosRestantes < 30 ? AppColors.danger : AppColors.warning;
    ...
    return Column(children: [
      Padding(
        ...
        child: Column(children: [
          Row(children: [
            Expanded(
                child: Text('${_items.length} ingresos encontrados',
          ...
          const SizedBox(height: 4),
          const Text(
              'Se guardará automáticamente al terminar el tiempo.',
              style: TextStyle(
                ...
                Expanded(
                  child: Text(
                    [
                      ...
                      if (_actualizados > 0)
                        '$_actualizados fijo${_actualizados == 1 ? '' : 's'} con monto actualizado',
                    ].join(' · '),
                    style: const TextStyle(
          ...
          if (liquidezList.isNotEmpty) ...[
            const SizedBox(height: 8),
            Row(children: [
              const Text('Acreditar en:', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
              ...
              Expanded(
                child: DropdownButtonFormField<String?>(
                  ...
                  items: [
                    ...
                    ...liquidezList.map((l) => DropdownMenuItem<String?>(
                      value: l.id,
                      child: Text('${l.tipo.emoji} ${l.nombre}', style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis),
                    )),
                  ],
      ...
      Expanded(
        child: ListView.separated(
          ...
          itemBuilder: (_, i) => _IngresoParseadoTile(
            item: _items[i],
      ...
      Padding(
        ...
        child: Column(children: [
          FilledButton.icon(
            ...
            label: Text(
                'Guardar ${_items.length} ingreso${_items.length == 1 ? '' : 's'}',
                style: const TextStyle(
                    fontSize: 15, fontWeight: FontWeight.w600)),
          ),
          const SizedBox(height: 8),
...
class _IngresoParseadoTile extends StatefulWidget {
  final IngresoParseado item;
  final List<Liquidez> liquidezList;
  final VoidCallback onEliminar;
  const _IngresoParseadoTile(
      {required this.item, required this.liquidezList, required this.onEliminar});

  @override
  State<_IngresoParseadoTile> createState() =>
      _IngresoParseadoTileState();
}

class _IngresoParseadoTileState extends State<_IngresoParseadoTile> {
  ...
  void initState() {
    ...
    _montoCtrl = TextEditingController(
        text: widget.item.monto.toStringAsFixed(2));
  }
  ...
  Color get _tipoColor => widget.item.tipo == TipoIngreso.fijo
      ? AppColors.info
      : AppColors.success;
                ...
                child: Column(
                  ...
                  Row(children: [
                    Container(
                      ...
                      child: Text(
                          widget.item.tipo == TipoIngreso.fijo
                              ? 'Fijo'
                              : 'Variable',
                          style: TextStyle(
                              color: _tipoColor,
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    _EditChip(
                        ...
                        color: AppColors.success,
                        onTap: () => setState(() => widget.item.tipo =
                            TipoIngreso.variable)),
                  ]),
                  const SizedBox(height: 10),
                  Row(children: [
                    ...
                    _EditChip(
                        label: 'ARS',
                        selected: !widget.item.esUSD,
                        color: AppColors.primary,
                        onTap: () => setState(
                            () => widget.item.esUSD = false)),
                    const SizedBox(width: 6),
                    _EditChip(
                        label: 'USD',
                        selected: widget.item.esUSD,
                        color: AppColors.primary,
                        onTap: () =>
                            setState(() => widget.item.esUSD = true)),
                  ]),
                  const SizedBox(height: 10),
                  InkWell(
                    onTap: () async {
                  ...
                  if (widget.liquidezList.isNotEmpty) ...[
                    ...
                    DropdownButtonFormField<String?>(
                      ...
                      items: [
                        const DropdownMenuItem<String?>(value: null, child: Text('Sin medio de pago', style: TextStyle(fontSize: 12))),
                        ...widget.liquidezList.map((l) => DropdownMenuItem<String?>(
                          value: l.id,
                          child: Text('${l.tipo.emoji} ${l.nombre}', style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis),
                        )),
                      ],
